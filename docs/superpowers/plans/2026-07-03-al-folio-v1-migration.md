...
## Workflow rules (all phases)

### Branch — why and how

> Caveman version:
>
> `master` = live site. GitHub Pages deploy from here. Don't break.
>
> Migration = big change. Need sandbox branch. Break stuff there, not on live.
>
> ```bash
> git checkout master
> git pull origin master
> git checkout -b upgrade/al-folio-v1
> ```
>
> Work only on `upgrade/al-folio-v1` until Phase 04 GHA green. Then merge → `master` → site update.
>
> Half-done merge = bad. Rollback = revert merge on `master`; old deploy still there.

**Branch naming:** one branch for whole migration (Phases 01–04). Content refresh (Phase 05) can stay on same branch or follow-up PR — your call at merge time.

### No random scripts — use theme tooling

al-folio v1 already ship what we need. Don't invent helper bash/python.

| Need | Use this | Don't create |
|------|----------|--------------|
| Local dev | `docker compose up` | custom `serve.sh` |
| Build | `docker compose run --rm jekyll bundle exec jekyll build` | `build.sh` |
| v1 migration | `bundle exec al-folio upgrade audit \| apply --safe \| report` | migration python script |
| Override drift | `bundle exec al-folio upgrade overrides audit \| diff \| accept` | diff scripts |
| Lockfile bootstrap | one-off `docker run … bundle lock` (Phase 01 only) | permanent `bootstrap.sh` in repo |
| Deploy | `.github/workflows/deploy.yml` (GHA) | `bin/deploy` on host |

**Cleanup targets** (Phase 01 — remove or stop using, don't replace with new scripts):

- `bin/deploy`, `bin/cibuild` — assume host `bundle`; wrong for Docker-only workflow
- `bin/docker_run.sh`, `bin/docker_build_image.sh`, `bin/dockerhub_run.sh` — pre-v1 image hacks
- `docker-local.yml` — duplicate compose; delete after v1 `docker-compose.yml` works
- Local `_plugins/*.rb` — gem replaced; delete, don't wrap

**Allowed new files from upstream only:** `bin/entry_point.sh`, v1 `Dockerfile`, `docker-compose.yml`, `.ruby-version`, `.al-folio-overrides.yml` (generated by upgrade CLI).

**Agent rule:** before adding any script, check if `al-folio upgrade *` or `docker compose` already do it. If yes → use that. No one-off automation in repo root.
...
- [ ] Include branch workflow + no-random-scripts policy (this plan section)
...
- [ ] Create migration branch (see **Workflow rules** above):
  ```bash
  git checkout master && git pull origin master
  git checkout -b upgrade/al-folio-v1
  ```
- [ ] Remove stale host-Ruby scripts: `bin/deploy`, `bin/cibuild`, `bin/docker_*.sh` (no replacements — use `docker compose` + GHA)
